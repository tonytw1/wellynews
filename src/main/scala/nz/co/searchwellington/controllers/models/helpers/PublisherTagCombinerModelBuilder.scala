...
@Component object PublisherTagCombinerModelBuilder {
  private[models] var logger: Logger = Logger.getLogger(classOf[PublisherTagCombinerModelBuilder])
}

@Component class PublisherTagCombinerModelBuilder extends ModelBuilder {
  ...
  @Autowired def this(contentRetrievalService: ContentRetrievalService, rssUrlBuilder: RssUrlBuilder, urlBuilder: UrlBuilder, relatedTagsService: RelatedTagsService, commonAttributesModelBuilder: CommonAttributesModelBuilder) {
    this()
  ...
  def populateContentModel(request: HttpServletRequest): ModelAndView = {
    if (isValid(request)) {
      PublisherTagCombinerModelBuilder.logger.info("Building publisher tag combiner page model")
      val tag: Tag = request.getAttribute("tag").asInstanceOf[Tag]
      val publisher: Website = request.getAttribute("publisher").asInstanceOf[Website]
