...
class FeednewsItemToNewsitemServiceTest {
  ...
  def shouldSetGeocodeWhenAcceptingFeedNewsitem {
    ...
    val feedNewsitem = new FrontendFeedNewsitem("Test feed item", "http://localhost", null, null, place, frontendFeed, null, null)

    val newsitem: Newsitem = service.makeNewsitemFromFeedItem(feed, feedNewsitem)

    assertEquals("A place", newsitem.geocode2.get.getAddress)
  }
